...
#include "smack/DSAAliasAnalysis.h"
    ...
    //RegisterPass<DSAAliasAnalysis> A("smack-ds-aa", "Data Structure Graph Based Alias Analysis");
    //RegisterAnalysisGroup<AliasAnalysis> B(A);
    //char DSAAliasAnalysis::ID = 0;

    char DSAAliasAnalysis::ID = 0;
    static RegisterPass<DSAAliasAnalysis> X("smack-ds-aa", "Smack - Data Structure Graph Based Alias Analysis", true, true);
    static RegisterAnalysisGroup<AliasAnalysis> Y(X);

    void DSAAliasAnalysis::getAnalysisUsage(AnalysisUsage &AU) const
    {
    ...
    vector<const llvm::DSNode*> DSAAliasAnalysis::collectMemcpys(
     ...
     bool DSAAliasAnalysis::runOnModule(llvm::Module &M) {
    ...
    llvm::AliasAnalysis::AliasResult DSAAliasAnalysis::alias(const Location &LocA, const Location &LocB) {
